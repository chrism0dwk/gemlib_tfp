...
class AdaptiveRandomWalkMetropolisHastings(kernel_base.TransitionKernel):
  ...
  """
    ...
    def update_covariance_scaling(self, prev_results, num_steps):        
        ...
        target_accept_ratio = tf.constant(
            self.target_accept_ratio, dtype=dtype
        )
            ...
            current_covariance_scaling = tf.gather(
                tf.stack(
                    [
                        ...
                        self.update_covariance_scaling(previous_kernel_results, num_steps),
                    ],
                    axis=-1,
                ),
    ...
    def bootstrap_results(self, init_state):
        ...
        with tf.name_scope(
            ...
            init_covariance_scaling = tf.cast(
                tf.repeat([self.initial_covariance_scaling], repeats=[shape[0]], axis=0),
                dtype=dtype,
            )
